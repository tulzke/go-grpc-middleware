...
func (r *reporter) PostCall(err error, rpcDuration time.Duration) {
	...
	switch r.kind {
	case KindServer:
		r.incrementWithExemplar(r.serverMetrics.serverHandledCounter, string(r.typ), r.service, r.method, code.String(), r.client)
		if r.serverMetrics.serverHandledHistogram != nil {
			r.observeWithExemplar(r.serverMetrics.serverHandledHistogram, rpcDuration.Seconds(), string(r.typ), r.service, r.method, r.client)
		}
...
func (r *reporter) PostMsgSend(_ any, _ error, sendDuration time.Duration) {
	switch r.kind {
	case KindServer:
		r.incrementWithExemplar(r.serverMetrics.serverStreamMsgSent, string(r.typ), r.service, r.method, r.client)
	case KindClient:
...
func (r *reporter) PostMsgReceive(_ any, _ error, recvDuration time.Duration) {
	switch r.kind {
	case KindServer:
		r.incrementWithExemplar(r.serverMetrics.serverStreamMsgReceived, string(r.typ), r.service, r.method, r.client)
	case KindClient:
...
func (rep *reportable) reporter(ctx context.Context, sm *ServerMetrics, cm *ClientMetrics, meta interceptors.CallMeta, kind Kind) (interceptors.Reporter, context.Context) {
	...
	r := &reporter{
		...
		method:        meta.Method,
		kind:          kind,
		client:        grpc_signature.ClientName(ctx),
	}
	if c.exemplarFn != nil {
	...
	case KindServer:
		r.incrementWithExemplar(r.serverMetrics.serverStartedCounter, string(r.typ), r.service, r.method, r.client)
	}
	return r, ctx
}
